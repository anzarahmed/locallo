...
interface DatePickerProps {
  ...
  /** Replaces the default input look (border, padding, radius); the calendar-icon spacing is kept */
...
export default function DatePicker({
  ...
  placeholder = 'DD/MM/YYYY', touched, error, required, disabled, hideErrorText, inputClassName,
        ...
        >
                ...
                <button
                  ...
                  className={`h-9 rounded-lg text-sm transition-colors ${
                    selected
                      ...
                      : `${cell.inMonth ? 'text-gray-800' : 'text-gray-400'} hover:bg-teal-50 ${
                          isToday ? 'ring-1 ring-inset ring-teal-400 font-semibold' : ''
                        }`
                  }`}
                >
                  {cell.day}
          ...
          <div className="flex items-center justify-between mt-2 pt-2 border-t border-gray-100">
            ...
            <button
              ...
              className="text-xs font-medium text-teal-600 hover:text-teal-800 px-2 py-1 rounded-md hover:bg-teal-50 disabled:text-gray-300 disabled:hover:bg-transparent transition-colors"
            >
              Today
            </button>
          </div>
  ...
  return (
    <div>
      {label && (
        <label htmlFor={name} className="text-xs font-medium text-gray-500 block mb-1.5">
          {label}
      ...
      <div className="relative">
        <input
          ...
          className={`pr-9 ${inputClassName ?? `w-full px-3 py-3 border rounded-xl text-sm text-gray-900 placeholder-gray-400
            outline-none transition-colors disabled:bg-gray-50 disabled:text-gray-400
            ${invalid ? 'border-red-400 bg-red-50' : 'border-gray-200 bg-white focus:border-teal-400'}`}`}
        />
        <button
          type="button"
          tabIndex={-1}
