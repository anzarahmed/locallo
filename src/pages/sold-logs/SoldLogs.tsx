...
import { getSoldLogs } from '../../services/sellerService';
...
export default function SoldLogs(): JSX.Element {
  ...
  const [logs, setLogs] = useState<SoldLog[]>([]);
  ...
  useEffect(() => {
    async function load(): Promise<void> {
      ...
      try {
        ...
        const data = await getSoldLogs({ page, limit: PAGE_LIMIT, ...range });
        setLogs(data.logs);
        setTotal(data.total);
      } catch (err) {
        toast.error(err instanceof ApiError ? err.message : 'Failed to load sales log');
      } finally {
        setLoading(false);
      }
    }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-white text-2xl font-bold leading-tight">Sales Log</h1>
            {!loading && (
    ...
    <div className="bg-white rounded-2xl shadow-sm p-4">
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="shrink-0 text-right">
          <div className="inline-flex items-center gap-1 bg-teal-600 text-white text-xs font-bold px-2.5 py-1 rounded-full">
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm py-16 text-center">
      ...
      <p className="text-xs text-gray-400 mt-1">
        Mark products or variants as sold to track your inventory
      </p>
    </div>
  );
}
